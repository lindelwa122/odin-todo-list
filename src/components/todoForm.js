  ...
  const show = () => {
    ...
    projects.forEach((project) => {
      domManager.create(
        ...
        '#todo-form-dialog select#project',
        true,
      );
    });
  ...
  const content = () => {
    ...
    const form = () => {
      ...
      const submitHandler = (e) => {
        const { id, title, description, priority, duedate, labels, project } =
        ...
        if (userInterfaceAPI.todoExists(id)) {
          ...
          userInterfaceAPI.updateTodo(id, 'project', {
            todoID: id,
            projectID: project,
          });
        } else {
      ...
      const d = new Date();
      const minDuedate = `${d.getFullYear()}-${
        d.getMonth() + 1
      }-${d.getDate()}`;
      ...
      return {
        ...
        children: [
          ...
          formField('label', {
            for: 'description',
            textContent: 'Describe your task',
          }),
          formField('textarea', {
            id: 'description',
            name: 'description',
          ...
          formField('label', {
            for: 'priority',
            textContent: 'How important is this task?',
          }),
          {
            tagName: 'select',
            options: {
          ...
          formField('label', {
            for: 'project',
            textContent: 'Where does this task belong?',
          }),
          formField('select', {
            id: 'project',
            name: 'project',
